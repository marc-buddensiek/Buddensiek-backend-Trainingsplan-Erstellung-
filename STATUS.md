...
| 9 | Claude-Integration | 🟡 9-1 + 9-3 + 9-3a ✓; 9-3b → MVP-10; offen: Output-Review (9-4) | **Naht 9-1 (2026-06-17):** Prompt **pseudonymisiert** — Name/Motivation/Stress/Schlaf raus, nur Alter/Level/Ziel/Equipment/Verletzungen (GDPR-Hebel, s. BACKLOG „ROBUSTHEIT & GO-LIVE", GDPR-Schritt 2). **Naht 9-3a (`980fd3e`):** Retry/Backoff (`MAX_VERSUCHE=3`, `BACKOFF=[2,5]s`) für API-/JSON-/Validierungs-/Vollständigkeits-Fehler im einheitlichen Retry-Pfad, danach `raise` (kein Pool-Fallback); `_pruefe_vollstaendigkeit` — SOLL nur Kraft-Sessions (Slot ohne `pool`-Marker), Subset-Prüfung + Slot-Anzahl. **Naht 9-3b → MVP-10 (vertagt):** Status-Persistenz (`failed` in DB) + `vorgang_id`-Rückgabe an Klienten + Abruf-Endpunkt (Schema-Arbeit; Marker `TODO(9-3b-status-persist)` am Pipeline-Fehlerpfad, main.py `_generiere_plan_task` except). **Naht 9-3 Prompt-Aktualisierung: erledigt (`0449e54`).** Übungs-Rendering um `name` + primäre `muscle_groups` erweitert; neue Auswahlprinzipien (Anspruch für Fortgeschrittene L3-4 → Compounds vor Isolation; Muskel-Belastung je Session ausbalancieren); Prinzip 3 bereinigt (toter `progressions_up`/`block_nummer>1`-Verweis raus → reine Block-Konstanz). **Pflicht-Patterns:** Pflicht-Befüllung jedes Slots ist über Slot-Vorgabe + 9-3a-Vollständigkeitscheck abgedeckt — explizite Pflicht-Pattern-Anweisung im Prompt bewusst NICHT ergänzt; am Output-Review prüfen, ob nötig. **Bibliothek-Ausbau (Coach):** `push_vertical`-L1-Lücke erledigt (`4a444e6`); Zug-Front (pull_vertical/pull_horizontal) geprüft — kein Content-Loch (nur dünn/kein L4; Stangen-Realismus geparkt, s. BACKLOG); `progressions_up`-Verlinkung bewusst vertagt (Konsument = geparkte „Progression V2"). **Offen:** nur noch echter **Output-Review (9-4)** — braucht verifizierten API-Key + Einzel-Testlauf. **Carry-Split: erledigt (`e97a83c`).** Equipment-bewusste Carry-Auflösung in `split_selector` (dünner `waehle_split`-Wrapper über `_waehle_split_impl`, keine Builder-Signatur-Änderung). Trigger = keine ECHTE Carry-Übung (carry-Pool ohne `ersatz_fuer`-Marker) via `filtere_uebungen` → respektiert Equipment + Level + Verletzung + `_EQUIPMENT_INCLUDES` (hybrid behält Carry). FB-C: Carry→Core (bleibt 4 Slots); Lower-B: Carry ersatzlos entfernt (6→5, behebt Doppel-Core, da Core dort separat existiert). Nur der carry-Fall; andere leere Pools bleiben laut über den 9-3a-Check; MVP-5-Fallback (carry→core) als Sicherheitsnetz unangetastet. **Level-Floor: KEIN Floor (entschieden)** — Level ist Selektions-Decke, kein Boden. L1-Basis-Pattern (z.B. Push-ups/Dips, tier=compound) gehören bewusst auch in L4-Hauptübungs-Slots; Trennung Basis vs. triviale Isolation läuft über `tier` (compound/accessory), nicht über die Level-Zahl. Forderung für Fortgeschrittene kommt aus Intensität (RPE/Last, MVP-6) + Übungs-Varianten, nicht aus einem Floor. **Analyse-Nicht-Löcher (entschieden):** `skill_level` ist Kompetenz-Gate (Schutz nach unten, Gate `<=`, oben offen → L4 sieht alle 158) → „nur 8 echte L4-Übungen" ist KEIN Loch; `conditioning` skaliert über Format/Intensität (nicht `skill_level`) → leere L3/L4-conditioning-Zellen sind KEIN Loch. Vorarbeit: Level-Gate als `<=` verifiziert (equipment_filter:73, vor Verletzungsfilter) | 5 ✓ |
...
- Hauptziel auf 4 Ziele (longevity statt ausdauer+gesundheit), tage_pro_woche ge=3, nebenziel + schmerzen_akut entfernt, schwachstelle-Feld ergänzt, session_typ/MetconBlock-Literale additiv erweitert, HauptUebung.rpe_hinweis + PlanMetadata-Submodell. Spec/Roadmap angelegt (CODE_CHANGES damals angelegt, 2026-06-30 archiviert/entfernt — überholt durch STATUS/BACKLOG).
