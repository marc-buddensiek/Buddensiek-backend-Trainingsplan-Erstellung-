...
8. [ERLEDIGT — Form bereits vorhanden] Sessions als geordnete Liste mit separatem tag-Feld (V2-Self-Service, zeitkritisch) — Wochentag NICHT in Inhalt backen → spätere Tag-Tausch-/Umlabel-Fähigkeit ohne Neugenerierung. 🔧 Struktur existiert schon: Session.tag ist ein eigenständiges, überschreibbares Feld; Frontend kann den Tag umlabeln (Di→Mi), Inhalt bleibt. Kein Bau nötig. Verifiziert an api_13 + models.py.
